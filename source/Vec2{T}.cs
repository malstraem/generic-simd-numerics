        ...
        where T : unmanaged, INumber<T>
{
    ...
    public static Vec2<T> UnitZ { get; } = new(T.Zero, T.Zero);

    public static Vec2<T> UnitW { get; } = new(T.Zero, T.Zero);

    #region Operators
    [MethodImpl(AggressiveInlining)]
    public static Vec2<T> operator +(Vec2<T> vec)
    {
        if (Unsafe.SizeOf<T>() == 4 && Vector64<T>.IsSupported)
            return Vec2<T>.From64(+vec.As64());

        if (Unsafe.SizeOf<T>() == 8 && Vector128<T>.IsSupported)
            return Vec2<T>.From128(+vec.As128());
    ...
    {
        if (Unsafe.SizeOf<T>() == 4 && Vector64<T>.IsSupported)
            return Vec2<T>.From64(vec.As64() + Vector64.Create(num));

        if (Unsafe.SizeOf<T>() == 8 && Vector128<T>.IsSupported)
            return Vec2<T>.From128(vec.As128() + Vector128.Create(num));
    ...
    {
        if (Unsafe.SizeOf<T>() == 4 && Vector64<T>.IsSupported)
            return Vec2<T>.From64(vec.As64() * num);

        if (Unsafe.SizeOf<T>() == 8 && Vector128<T>.IsSupported)
            return Vec2<T>.From128(vec.As128() * num);
    ...
    {
        if (Unsafe.SizeOf<T>() == 2 && Vector64<T>.IsSupported)
        {
            var res = Vector64.LoadUnsafe(ref left.X) + Vector64.LoadUnsafe(ref right.X);

            return Unsafe.As<Vector64<T>, Vec2<T>>(ref res);
        }

        if (Unsafe.SizeOf<T>() == 4 && Vector64<T>.IsSupported)
            return Vec2<T>.From64(left.As64() + right.As64());

        if (Unsafe.SizeOf<T>() == 8 && Vector128<T>.IsSupported)
            return Vec2<T>.From128(left.As128() + right.As128());
    ...
    {
        if (Unsafe.SizeOf<T>() == 4 && Vector64<T>.IsSupported)
            return Vec2<T>.From64(Vector64.Abs(As64()));

        if (Unsafe.SizeOf<T>() == 8 && Vector128<T>.IsSupported)
            return Vec2<T>.From128(Vector128.Abs(As128()));
    ...
    {
        if (Unsafe.SizeOf<T>() == 4 && Vector64<T>.IsSupported)
            return Vec2<T>.From64(Vector64.Min(As64(), vec.As64()));

        if (Unsafe.SizeOf<T>() == 8 && Vector128<T>.IsSupported)
            return Vec2<T>.From128(Vector128.Min(As128(), vec.As128()));
    ...
    {
        if (Unsafe.SizeOf<T>() == 4 && Vector64<T>.IsSupported)
            return Vec2<T>.From64(Vector64.Max(As64(), vec.As64()));

        if (Unsafe.SizeOf<T>() == 8 && Vector128<T>.IsSupported)
            return Vec2<T>.From128(Vector128.Max(As128(), vec.As128()));
    ...
    {
        if (Unsafe.SizeOf<T>() == 4 && Vector64<T>.IsSupported)
            return Vec2<T>.From64(Vector64.Clamp(As64(), min.As64(), max.As64()));

        if (Unsafe.SizeOf<T>() == 8 && Vector128<T>.IsSupported)
            return Vec2<T>.From128(Vector128.Clamp(As128(), min.As128(), max.As128()));
    ...
    public readonly Vec2<T> Lerp(Vec2<T> vec, T amount)
    {
        ...
        {
            return Vec2<T>.From64(Vector64.Lerp
            (
                As64F(),
                vec.As64F(),
        ...
        {
            return Vec2<T>.From128(Vector128.Lerp
            (
                As128D(),
                vec.As128D(),
    ...
    public readonly Vec2<T> SquareRoot<TRoot>() where TRoot : IRootFunctions<TRoot>
    {
        ...
        if (Unsafe.SizeOf<T>() == 4 && Vector64<T>.IsSupported)
            return Vec2<T>.From64(Vector64.Sqrt(As64()));

        if (Unsafe.SizeOf<T>() == 8 && Vector128<T>.IsSupported)
            return Vec2<T>.From128(Vector128.Sqrt(As128()));
