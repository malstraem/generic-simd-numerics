...
{
    extension<T>(Vec3<T> vec) where T : unmanaged, INumber<T>, IRootFunctions<T>
    {
...
public static class Vec3StaticExtensions // god that extensions behavior sucks
{
    extension<T>(Vec3<T>) where T : unmanaged, INumber<T>, IRootFunctions<T>
    {
        [MethodImpl(AggressiveInlining)]
