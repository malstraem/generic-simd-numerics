...
public static partial class Mat44
{
        ...
        where T : unmanaged, INumber<T>, IRootFunctions<T>, ITrigonometricFunctions<T>
    {
        ...
        x = mx.Permute32(1, 2, 0, 3).Estimate(k, x); // mx(y, z, x, w) * (q21, q32, q13, -) + ...
        x = mx.Permute32(2, 0, 1, 3).Estimate(n, x); // mx(z, x, y, w) * (q31, q12, q23, -) + ...

        y = my * w;                                     // my(x, y, z, w) * (q11, q22, q33, -)
        y = my.Permute32(1, 2, 0, 3).Estimate(k, y); // my(y, z, x, w) * (q21, q32, q13, -) + ...
        y = my.Permute32(2, 0, 1, 3).Estimate(n, y); // my(z, x, y, w) * (q31, q12, q23, -) + ...

        z = mz * w;                                     // mz(x, y, z, w) * (q11, q22, q33, -)
        z = mz.Permute32(1, 2, 0, 3).Estimate(k, z); // mz(y, z, x, w) * (q21, q32, q13, -) + ...
        z = mz.Permute32(2, 0, 1, 3).Estimate(n, z); // mz(z, x, y, w) * (q31, q12, q23, -) + ...

        w = mw * w;                                     // mw(x, y, z, w) * (q11, q22, q33, -)
        w = mw.Permute32(1, 2, 0, 3).Estimate(k, w); // mw(y, z, x, w) * (q21, q32, q13, -) + ...
        w = mw.Permute32(2, 0, 1, 3).Estimate(n, w); // mw(z, x, y, w) * (q31, q12, q23, -) + ...
        ...
        where T : unmanaged, INumber<T>, IRootFunctions<T>, ITrigonometricFunctions<T>
    {
        ...
        x = mx * w;                                     // mx(x, y, z, w) * (q11, q22, q33, -)
        x = mx.Permute64(1, 2, 0, 3).Estimate(k, x); // mx(y, z, x, w) * (q21, q32, q13, -) + ...
        x = mx.Permute64(2, 0, 1, 3).Estimate(n, x); // mx(z, x, y, w) * (q31, q12, q23, -) + ...

        y = my * w;                                     // my(x, y, z, w) * (q11, q22, q33, -)
        y = my.Permute64(1, 2, 0, 3).Estimate(k, y); // my(y, z, x, w) * (q21, q32, q13, -) + ...
        y = my.Permute64(2, 0, 1, 3).Estimate(n, y); // my(z, x, y, w) * (q31, q12, q23, -) + ...

        z = mz * w;                                     // mz(x, y, z, w) * (q11, q22, q33, -)
        z = mz.Permute64(1, 2, 0, 3).Estimate(k, z); // mz(y, z, x, w) * (q21, q32, q13, -) + ...
        z = mz.Permute64(2, 0, 1, 3).Estimate(n, z); // mz(z, x, y, w) * (q31, q12, q23, -) + ...

        w = mw * w;                                     // mw(x, y, z, w) * (q11, q22, q33, -)
        w = mw.Permute64(1, 2, 0, 3).Estimate(k, w); // mw(y, z, x, w) * (q21, q32, q13, -) + ...
        w = mw.Permute64(2, 0, 1, 3).Estimate(n, w); // mw(z, x, y, w) * (q31, q12, q23, -) + ...
